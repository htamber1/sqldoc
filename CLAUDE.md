...
Shipped, **MIT-licensed, and live on PyPI** (`pip install sqldoc`) — a seven-command CLI: **`sqldoc doc`** (documentation), **`sqldoc scan`** (PII/compliance), **`sqldoc health`** (DMV health), **`sqldoc quality`** (data profiling), **`sqldoc intel`** (schema intelligence), **`sqldoc insights`** (AI insights), **`sqldoc comply`** (compliance expansion). Release tags **v1.0.0 → v1.6.0** pushed to `github.com/htamber1/sqldoc` (latest **v1.6.0**, MIT license). **232 pytest tests** passing (mocked — no live database/Ollama). **CI is live** on GitHub (`.github/workflows/main.yml`). **Multi-database `DatabaseAdapter` layer** with **six** dialects — **SQL Server, Azure SQL, PostgreSQL, MySQL, SQLite, Snowflake**. `doc`/`scan`/`intel`/`insights` run on all six; **`health`/`quality` run on SQL Server + PostgreSQL + MySQL** (v1.6.0); `comply` access-audit stays SQL-Server/Azure only. **Live-validated in Docker**: Pagila (PostgreSQL 16), Sakila (MySQL 8), Chinook (SQLite). Snowflake is **mock-tested only**. See "Multi-database (v1.5.x–v1.6.0)" below. Validated end-to-end against a local `AdventureWorks2022` (71 tables / 20 views / 10 procs / 10 triggers / 10 computed columns; `sa`/`SqlDoc123!`). Version is single-sourced in `sqldoc/__init__.py` (`__version__`) — `cli.py` banners + `--version` and `sarif.py` read it; `pyproject.toml` is the only other place to bump.
...
- **GitHub Release pages for `v1.5.0` / `v1.5.1`** (paste-ready notes are the CHANGELOG entries) — and still the older `v1.2.0`/`v1.3.0` pages.
- **Live-DB validation done for Postgres + MySQL (v1.5.1).** Remaining: validate **Azure SQL** against a real instance; optionally add a docker-compose + a `make validate-adapters` harness to re-run the Pagila/Sakila checks in CI.
