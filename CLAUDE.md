...
Shipped, **MIT-licensed, and live on PyPI** (`pip install sqldoc`) — a seven-command CLI: **`sqldoc doc`** (documentation), **`sqldoc scan`** (PII/compliance), **`sqldoc health`** (DMV health), **`sqldoc quality`** (data profiling), **`sqldoc intel`** (schema intelligence), **`sqldoc insights`** (AI insights), **`sqldoc comply`** (compliance expansion). Release tags **v1.0.0 → v1.7.0** pushed to `github.com/htamber1/sqldoc` (latest **v1.7.0**, MIT license). **292 pytest tests** passing (mocked — no live database/Ollama). **v1.7.0 added `sqldoc agent`** — a persistent background monitoring daemon (SQLite state in `~/.sqldoc/agent.db`, per-database poller threads, a localhost:8080 dashboard, Slack/email alerts). See "sqldoc agent (v1.7.0)" below. **CI is live** on GitHub (`.github/workflows/main.yml`). **Multi-database `DatabaseAdapter` layer** with **seven** dialects — **SQL Server, Azure SQL, PostgreSQL, MySQL, SQLite, Snowflake, Oracle**. `doc`/`scan`/`intel`/`insights` run on all seven; **`health`/`quality`/`comply` (access-audit + lineage) run on SQL Server + PostgreSQL + MySQL** (comply lineage is dialect-neutral so it runs everywhere with definitions). **Live-validated in Docker**: Pagila (PostgreSQL 16), Sakila (MySQL 8), Chinook (SQLite) — doc/scan/intel/quality/health/comply. **Snowflake + Oracle are mock-tested only** (need a live account/license). See "Multi-database (v1.5.x–v1.6.1)" below. **Tomorrow: go-to-market prep — see "Next session" at the bottom.** Validated end-to-end against a local `AdventureWorks2022` (71 tables / 20 views / 10 procs / 10 triggers / 10 computed columns; `sa`/`SqlDoc123!`). Version is single-sourced in `sqldoc/__init__.py` (`__version__`) — `cli.py` banners + `--version` and `sarif.py` read it; `pyproject.toml` is the only other place to bump.
