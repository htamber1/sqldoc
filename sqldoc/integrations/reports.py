...
def bundle_markdown(bundle, metrics=None) -> str:
    """Render a bundle as a Markdown page (for wiki / Nuclino destinations)."""
    from sqldoc.pii import summarize as pii_summarize
    m = metrics or {}
    ...
    lines = [f"# Database: {bundle.database}", "", "_Generated by sqldoc._", ""]
    lines += ["## Executive scorecard", "",
              "| Overall | Data protection | Backups | Security | Performance |",
              "|---|---|---|---|---|",
              f"| {sc(m.get('overall_score'))} | {sc(m.get('pii_safety_score'))} | "
              f"{('N/A' if m.get('backup_compliance_pct') is None else str(m['backup_compliance_pct']) + '%')} | "
              f"{sc(m.get('security_score'))} | {sc(m.get('health_score'))} |", ""]
    lines += ["## Documentation", "",
              f"- Tables: {len(bundle.tables)}",
              f"- Views: {len(bundle.views)}",
              f"- Procedures: {len(bundle.procedures)}",
              f"- Sensitive columns: {len(bundle.findings)}", ""]

    pii = pii_summarize(bundle.findings)
    lines += ["## PII / compliance findings", "",
              f"{pii['total']} finding(s) across {pii['tables_affected']} table(s): "
              f"{pii['by_risk'].get('HIGH', 0)} HIGH, {pii['by_risk'].get('MEDIUM', 0)} MEDIUM, "
              f"{pii['by_risk'].get('LOW', 0)} LOW.", ""]
    if bundle.findings:
        lines += ["| Column | Category | Risk | Regulations | Action |", "|---|---|---|---|---|"]
        ordered = sorted(bundle.findings,
                         key=lambda f: ({"HIGH": 0, "MEDIUM": 1, "LOW": 2}.get(f.risk, 3),
                                        f.schema, f.table, f.column))[:200]
        for f in ordered:
            lines.append(f"| {f.schema}.{f.table}.{f.column} | {f.category} | {f.risk} | "
                         f"{', '.join(f.regulations or [])} | {getattr(f, 'action', '')} |")
        lines.append("")

    if bundle.health_summary:
        hs = bundle.health_summary
        lines += ["## Health summary", "",
                  f"- Slow queries: {hs.get('slow_queries')}",
                  f"- Missing indexes: {hs.get('missing_indexes')}",
                  f"- Total issues: {hs.get('issues')}", ""]
    return "\n".join(lines) + "\n"
