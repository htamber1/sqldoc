...
def _index_summary(shape) -> str:
    """Human-readable shape of a snapshot index entry, e.g. 'UNIQUE (Id) INCLUDE (Total)'."""
    cols = ", ".join(f"[{c}]" for c in shape.get("columns") or [])
    parts = []
    if shape.get("primary_key"):
        parts.append("PRIMARY KEY")
    elif shape.get("unique"):
        parts.append("UNIQUE")
    parts.append(f"({cols})" if cols else "(columns unknown)")
    included = ", ".join(f"[{c}]" for c in shape.get("included") or [])
    if included:
        parts.append(f"INCLUDE ({included})")
    return " ".join(parts)
...
def generate_migration(old_snapshot: dict, new_snapshot: dict) -> str:
    ...
    Types come from the snapshot, which records a fully-qualified signature
    (`varchar(50)`, `decimal(18,4)`) wherever the adapter supplies width — and
    falls back to the bare type name where it does not. The script is still a
    **review starting point**, not a drop-in migration.
    """
    ...
    out = ["-- Migration generated by sqldoc (review before running).",
           "-- Types carry length/precision where the adapter reports it; verify the rest.",
    ...
    for mod in diff["tables_modified"]:
        ...
        for name in mod.get("uniques_removed", []):
            out.append(f"ALTER TABLE {q} DROP CONSTRAINT [{name}];")
        # Index changes are reported as comments, never scripted. The snapshot
        # records constraint-backing indexes under the constraint's own name, so
        # emitting CREATE/DROP INDEX here would collide with the PRIMARY KEY and
        # UNIQUE statements generated just above -- two conflicting statements
        # for one object, in a script a human is meant to run. A named comment
        # carrying the columns is the useful, non-conflicting output. Without
        # this, an index-only change produced a bare blank entry saying nothing.
        idx = new_t.get(mod["name"], {}).get("indexes", {})
        for name in mod.get("indexes_added", []):
            out.append(f"-- {q} index [{name}] added: "
                       f"{_index_summary(idx.get(name, {}))} (create manually)")
        for name in mod.get("indexes_removed", []):
            out.append(f"-- {q} index [{name}] removed (drop manually)")
        for ch in mod.get("indexes_changed", []):
            for fld, old_v, new_v in ch["deltas"]:
                out.append(f"-- {q} index [{ch['name']}]: {fld} {old_v} -> {new_v} "
                           f"(manual review)")
        out.append("")
