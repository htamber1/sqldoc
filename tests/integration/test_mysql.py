"""Phase 5 — live MySQL integration tests against Sakila.

Provision with: bash tests/integration/docker/setup_mysql.sh
Runs the dialect-neutral + MySQL-supported command suite end-to-end and
validates the HTML/JSON output. Skips cleanly when Sakila is unreachable."""
import json
import os

import pytest

from _live import MYSQL_CS, requires_mysql, run

pytestmark = [requires_mysql, pytest.mark.integration]

BASE = ["--connection-string", MYSQL_CS, "--dialect", "mysql"]
...
def test_doc_html_and_json(tmp_path):
    ...
    assert len(data["tables"]) >= 15       # Sakila has ~16 base tables
...
def test_scan_finds_pii(tmp_path):
    """Sakila's customer/staff tables have email + address PII."""
